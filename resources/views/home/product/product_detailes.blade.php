            ...
            <div class="col-lg-6 col-md-6 order-2 order-sm-2 order-md-1" style="direction: rtl;">
                <div class="product-details-content ml-30">
                    ...
                    <div class="pro-details-rating-wrap">
                        <div id="dataReadonlyReview" data-rating-stars="5" data-rating-readonly="true" data-rating-value="{{ceil($product->rates->avg('rate'))}}"></div>
                            ...
                            <div class="pro-details-size w-50">
                                ...
                                <select class="form-control variation-select">
                                    @foreach($product->variations()->where('quantity','>',0)->get() as $variation)
                                        <option value="{{json_encode($variation->only(['id','is_sale','sale_price','price','quantity'])) }}"{{ $variationProductSelected->id == $variation->id ? 'selected' : '' }}>{{$variation->value}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
            ...
            <div class="col-lg-6 col-md-6 order-1 order-sm-1 order-md-2">
                <div class="product-details-img">
                    <div class="zoompro-border zoompro-span">
                        <img class="zoompro" width="570" height="570"  src="{{Storage::url($product->primary_image)}}" data-zoom-image="{{Storage::url($product->primary_image)}}" alt="" />
                    </div>
                    <div id="gallery" class="mt-20 product-dec-slider">
                        <a data-image="{{Storage::url($product->primary_image)}}" data-zoom-image="{{Storage::url($product->primary_image)}}">
                            <img width="90" height="140" src="{{Storage::url($product->primary_image)}}" alt="">
                        </a>
                        @foreach ($product->files as $image)
                        <a data-image="{{ Storage::url($image->path.'/'.$image->name)}}" data-zoom-image="{{ Storage::url($image->path.'/'.$image->name)}}">
                            <img width="90"  height="140" src="{{ Storage::url($image->path.'/'.$image->name)}}" alt="">
                        </a>
                        @endforeach
                    </div>
                </div>
    ...
    <div class="container">
        ...
        <div class="arrivals-wrap scroll-zoom">
            <div class="ht-products product-slider-active owl-carousel">
                        @foreach($product_category->products as $product2)
                            @include('home.front.sections.product',['product'=>$product2])
                        @endforeach
            </div>
        </div>
    </div>
